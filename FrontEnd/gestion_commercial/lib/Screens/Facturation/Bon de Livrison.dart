...
class BonDeLivraisonTable extends StatefulWidget {
 var idEntreprise;
BonDeLivraisonTable({this.idEntreprise});
  @override
  _BonDeLivraisonTableState createState() => _BonDeLivraisonTableState();
}

class _BonDeLivraisonTableState extends State<BonDeLivraisonTable> {
            ...
            children: [
              ...
              child:Row(children: [
               Expanded(
                  ...
                  child: Text('Bon de livraison',style:TextStyle(fontSize: 25,fontWeight: FontWeight.bold, fontFamily: 'Barlow')),
                )),
               Spacer(flex:1),
                   ...
                   children: [
                     Row(
                       children: [
                         Padding(
                           padding: const EdgeInsets.only(left:8.0),
                           child: Text('Les  bon de livraison',style:TextStyle(fontSize: 19,fontWeight: FontWeight.bold, fontFamily: 'Barlow')),
                         ),
                       ],
...
Navigator.push(context, MaterialPageRoute(builder: (context)=>showEditDialog())); 
   ...
   },icon: Icon(Icons.add), label:Text ('Editer Bon de livraison'),style: ElevatedButton.styleFrom(primary: Colors.primaryColor )),
),
              ...
              DataRow(cells: [
                ...
                DataCell( SizedBox( width: double.infinity,child:Text(t.data['GetBonDeCommande'][i]['Commande']['id'].toString(),style:Responsive.isDesktop(context)? Textstyle():TextStyle(fontFamily: 'Barlow',fontSize: 18))) ),
 ...
 StatefulWidget showEditDialog(){
    return
      StatefulBuilder(
              ...
              child: Text('Nouveau Bon de livraison',style:TextStyle(fontSize: 25,fontWeight: FontWeight.bold, fontFamily: 'Barlow')),
...
selectedIndexes.add(i);
  print(stockslectedArticles);
      ...
      print(selectedIndexes);
             ...
             DataCell( SizedBox( width: double.infinity,child: TextField(keyboardType: TextInputType.number,decoration: InputDecoration(hintText: 'qtt',enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color:Colors.primaryColor ))),controller: commentController,)), ),
             ...
             DevisM commandes= DevisM(idEntreprise:widget.idEntreprise,idClient:idclient,qtt: selectedQtt,idArticles: stockslectedArticles);
                        ...
                        var x=await commandes.CreateDevis();
                         ...
                         if(x.data['CreateDevis']==1){
                           ...
                           showDialog(context: context, builder: (context)=>Dialog(child: Center(child: Container( child: Text('Devis Edité avec succés',style: Textstyle(),),))));
                         }
                        }else{
